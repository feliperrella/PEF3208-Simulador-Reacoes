<template>
  ...
  <v-row no-gutters align="stretch">
    ...
    <v-col cols="8">
      ...
      <div class="card-render" id="render-p2">
        <div ref="label-holder-2" class="label-holder">
        </div>
      </div>
    ...
    <v-col cols="4">
      ...
      <v-card class="mx-auto card-info" outlined>
        ...
        ></script>
        <h1>Viga em balanço (Peso distribuido)</h1>
        <div v-if="devTools">
...
export default {
  ...
  computed: {
    fma() {
      //f \cdot da - rb \cdot l
      return (
        "$$ {M}_A = \\frac{Pl^{2}}{2} \\rightarrow \\frac{" + Math.round(this.f * 100) / 100 + "\\cdot" + Math.round((this.sizeAvailable / 100) * 100) / 100 +  "^{2}}{2}"  + " =" + Math.round((this.ma / 100) * 100) + "$$"
      );
    },
    fya() {
      return (
        "$$ {Y}_A = F \\cdot l \\rightarrow " +
  ...
  data() {
    return {
      ...
      sumFy: "$$\\sum {F}_y = 0 \\rightarrow {Y}_A = F \\cdot l $$",
      sumMa: "$$\\sum {M}_A = 0 \\rightarrow {M}_A = \\frac{F \\cdot l^{2}}{2} $$",
      bodiesDom: [],
      bodies: [],
      bodiesPositions: [
  ...
  async mounted() {
    ...
    this.height = document.getElementById("render-p2").clientHeight;
    this.width = document.getElementById("render-p2").clientWidth;
    let engine = Matter.Engine.create();
    let render = Matter.Render.create({
      element: document.querySelector("#render-p2"),
      engine: engine,
      options: {
        height: this.height,
        width: this.width,
    ...
    this.bodiesDom = document.querySelectorAll(".label2");
    this.bodies = [];
  ...
  methods: {
    ...
    calculateReactions(y) {
      if (this.inRange) {
        ...
        this.ya = (this.f * this.sizeAvailable) / 100;
        this.ma = ((this.f) * ((this.sizeAvailable / 100) * (this.sizeAvailable / 100)))/2
    ...
    createLabel(width, height,x,y, id, text) {
      ...
      this.$refs['label-holder-2'].appendChild(label)
    },
    onResize(){
      console.log("resized")
    }
