<template>
  ...
  <v-row no-gutters align="stretch">
    ...
    <v-col cols="8">
      ...
      <div class="card-render" id="render-p5">
        <div id="label-holder-5" class="label-holder">
        </div>
      </div>
    ...
    <v-col cols="4">
      ...
      <v-card class="mx-auto card-info" outlined>
        ...
        ></script>
        <h1>Viga bi-apoiada (Peso distribuido)</h1>
        <div v-if="devTools">
        ...
        <div v-if="!devTools">
          ...
          <VueMathjax v-if="f != 0" :formula="fyb"></VueMathjax>
...
export default {
  ...
  data() {
    return {
      ...
      rb: 0,
      f: 0,
      sumFx: "$$\\sum {F}_X = 0 \\rightarrow {X}_B = 0 $$",
      sumFy: "$$\\sum {F}_Y = 0 \\rightarrow {Y}_A + {Y}_B = F \\cdot l \\rightarrow {Y}_A = F \\cdot l -  \\frac{F \\cdot l }{2} = \\frac{F \\cdot l }{2} $$",
      sumMa: "$$\\sum {M}_a = 0 \\rightarrow \\frac{F \\cdot l^{2} }{2} = {Y}_B \\cdot l \\rightarrow {Y}_B = \\frac{F \\cdot l}{2} $$",
      devTools: false,
      bodiesDom: [],
      bodies: [],
      bodiesPositions: [
  ...
  computed: {
    fyb() {
      //f \cdot da - rb \cdot l
      return (
        "$$ {Y}_B = \\frac{" + Math.round(this.f * 100) / 100 + "\\cdot" + (Math.round(this.sizeAvailable * 100) / 100) / 100 + "}{2} =" + (Math.round(this.ya * 100) / 100)  + " $$"
      );
    },
    fya() {
      return (
        "$$ {Y}_A = \\frac{" + Math.round(this.f * 100) / 100 + "\\cdot" + (Math.round(this.sizeAvailable * 100) / 100) / 100 + "}{2} =" + (Math.round(this.ya * 100) / 100)  + " $$"
      );
    },
    distances(){
      return(
  ...
  async mounted() {
    ...
    console.log(document.querySelector("#render-p5").offsetWidth)

    this.height = document.getElementById("render-p5").clientHeight;
    this.width = document.getElementById("render-p5").clientWidth;
    let engine = Matter.Engine.create();
    let render = Matter.Render.create({
      element: document.querySelector("#render-p5"),
      engine: engine,
      options: {
        height: this.height,
        width: this.width,
    ...
    //this.bodiesDom = []
    this.bodiesDom = document.querySelectorAll(".label-p5");
  ...
  methods: {
    ...
    createLabel(width, height,x,y, id, text) {
      ...
      document.getElementById("label-holder-5").appendChild(label);
    },
  },
};
</script>
